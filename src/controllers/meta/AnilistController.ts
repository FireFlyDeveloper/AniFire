...
export default class AnilistController {
  ...
  async search(c: AppContext) {
    try {
      ...
      if (!search) {
        return c.json({ error: "Missing required parameter: search" }, 400);
      }
      ...
      if (!type || (type !== "ANIME" && type !== "MANGA")) {
        return c.json(
          {
            error:
              "Invalid or missing parameter: type (must be ANIME or MANGA)",
          },
          400,
        );
      }
      ...
      const results = await AnilistService.getSearchResults(
        search,
        type,
        page,
        perPage,
    ...
    try {
      ...
      if (!idParam || isNaN(id) || id <= 0) {
        return c.json(
          {
            error:
              "Missing or invalid parameter: id (must be a positive integer)",
          },
          400,
        );
      }
